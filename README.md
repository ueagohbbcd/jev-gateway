...
`temperature` 调整概率后处理温度。空呼号列表使用 A–Z；非空列表替换这些字母。Noul 始终执行一次 `Yes`／`No` 判断。双循环用于 Choice 和 Score，每对候选交换位置各调用一次。

提示词是带四个占位符的普通文本，可以直接修改措辞、顺序和重复次数。变量值替换一次，证据正文原样保留。

每份配置独立生效，省略项使用代码默认值，未知配置项在加载前报错。运行中的服务可通过 stdin 重载整份配置；每次响应携带该请求所用配置的摘要 `x-jev-config-id`，方便关联实验。密钥通过环境变量提供。
...
新配置通过本地验证后整体切换，失败继续使用旧配置和旧路径。在途 HTTP 请求保留收到请求时的配置，新请求使用新配置。`[server]` 下的监听、鉴权和容量设置变更通过重启生效。
